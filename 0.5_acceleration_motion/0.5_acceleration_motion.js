...
*/
  ...
  //加速度ベクトル
  if(document.getElementById('avector1').checked){
    //加速度ベクトルの棒
    ctx.beginPath();
    ctx.moveTo(x, y+ballRadius*3);
    ctx.lineTo(x+ballRadius*2*acceleration, y+ballRadius*3);
    if(0 <= acceleration){
    //加速度正の時の矢印
    ctx.lineTo(x+ballRadius*2*acceleration+10*Math.cos(Math.PI/4*3), y+ballRadius*3-10*Math.sin(Math.PI/4*3));
    ctx.lineTo(x+ballRadius*2*acceleration+10*Math.cos(-Math.PI/4*3), y+ballRadius*3-10*Math.sin(-Math.PI/4*3));
    ctx.lineTo(x+ballRadius*2*acceleration, y+ballRadius*3);
  }else{
    //加速度負のときの矢印
...
//  if(250 < 600-v*t/3){
    ...
    ctx.lineTo(x+ballRadius+(v+acceleration*t*10), y+ballRadius*5);
...
/*}else{
  ...
  //速度ベクトルの矢印の先（上の画面）
  ctx.beginPath();
if(0 <= v+acceleration*t*10){
//  if(250 < 600-v*t/3){
    ctx.moveTo(x+ballRadius+(v+acceleration*t*10)+5, y+ballRadius*5);
//    ctx.moveTo(x+ballRadius+ballRadius*2+((v+acceleration*t*10)-1)*25, y+ballRadius*5);
    ctx.lineTo(x+ballRadius+(v+acceleration*t*10)+20*Math.cos(Math.PI/4*3)+5, y+ballRadius*5-20*Math.sin(Math.PI/4*3));
    ctx.lineTo(x+ballRadius+(v+acceleration*t*10)+20*Math.cos(-Math.PI/4*3)+5, y+ballRadius*5-20*Math.sin(-Math.PI/4*3));
...
/*}else{
  ...
  ctx.beginPath();
//  if(250 < 600-v*t/3){
    ctx.moveTo(ballRadius, ballRadius*6);
    ctx.lineTo(x+ballRadius-5, ballRadius*6);
/*}else{
    t1 = (canvas.width-ballRadius*2)/v
...
//  if(250 < 600-v*t/3){
  ...
  //変位の正・負で矢印の向きを変える
  if(0 <= x){
    ctx.moveTo(x+ballRadius, ballRadius*6);
    ctx.lineTo(x+ballRadius+20*Math.cos(Math.PI/4*3), ballRadius*6-20*Math.sin(Math.PI/4*3));
    ctx.lineTo(x+ballRadius+20*Math.cos(-Math.PI/4*3), ballRadius*6-20*Math.sin(-Math.PI/4*3));
}else{
    ctx.moveTo(x+ballRadius/2, ballRadius*6);
    ctx.lineTo(x+ballRadius/2-20*Math.cos(Math.PI/4*3), ballRadius*6-20*Math.sin(Math.PI/4*3));
    ctx.lineTo(x+ballRadius/2-20*Math.cos(-Math.PI/4*3), ballRadius*6-20*Math.sin(-Math.PI/4*3));
}
...
function drawfunction(){
  ...
  /*  ctx.beginPath();
    ctx.moveTo(100,600);
    if(250 < 600-v*t/3){
    ctx.lineTo(100+30*t,600-(v+acceleration*t*5)*t/3);
  }else{
  t1 = (canvas.width-ballRadius*2)/v
    ctx.lineTo(100+30*t1,600-(v+acceleration*t*5)*t1/3);
  }
    ctx.strokeStyle = 'blue';
    ctx.lineWidth = 5;
    //ctx.stroke();
    ...
    ctx.closePath();
*/
    //直線ではなく，曲線で一点一点表示されるようになる(等加速度直線運動のため)
    for(i = 0; i < t; i+= 0.1){
    ctx.beginPath();
    ctx.arc(100+30*i, 600-(v+acceleration*i*5)*i/3, 3, 0, Math.PI*2, false);
    ctx.fillStyle = 'blue';
    ctx.fill();
    ctx.fillStyle = 'black';
    ctx.closePath();
...
//  if(250 < 600-v*t/3){
  for(i = 0; i < t; i++){
  ctx.beginPath();
  ctx.arc(100+30*i, 600-(v+acceleration*i*5)*i/3, 5, 0, Math.PI*2, false);
  ctx.fillStyle = 'black';
  ctx.fill();
  ctx.closePath();
}//for
...
t1 = (canvas.width-ballRadius*2)/v
  ...
  ctx.lineTo(100+30*t1,600-(v+acceleration*t*5)*t1/3+5);
}*/
  ctx.strokeStyle = 'blue';
  ctx.lineWidth = 5;
  ctx.stroke();
...
//  if(250 < 600-v*t/3){
  if(0 <= x){
    ctx.moveTo(100+30*t,600-(v+acceleration*t*5)*t/3);
    ctx.lineTo(100+30*t+15*Math.cos(-Math.PI/3*2),600-(v+acceleration*t*5)*t/3-15*Math.sin(-Math.PI/3*2));
    ctx.lineTo(100+30*t+15*Math.cos(-Math.PI/3),600-(v+acceleration*t*5)*t/3-15*Math.sin(-Math.PI/3));
  }else{
    ctx.moveTo(100+30*t,600-(v+acceleration*t*5)*t/3+5);
    ctx.lineTo(100+30*t+15*Math.cos(-Math.PI/3*2),600-(v+acceleration*t*5)*t/3+15*Math.sin(-Math.PI/3*2)+5);
    ctx.lineTo(100+30*t+15*Math.cos(-Math.PI/3),600-(v+acceleration*t*5)*t/3+15*Math.sin(-Math.PI/3)+5);
  }
/*}else{
  t1 = (canvas.width-ballRadius*2)/(v+acceleration*t*5)
    ctx.moveTo(100+30*t1,600-v*t1/3);
    ctx.lineTo(100+30*t1+15*Math.cos(-Math.PI/3*2),600-(v+acceleration*t*5)*t1/3-15*Math.sin(-Math.PI/3*2));
    ctx.lineTo(100+30*t1+15*Math.cos(-Math.PI/3),600-(v+acceleration*t*5)*t1/3-15*Math.sin(-Math.PI/3));
}*/
  ctx.fillStyle = 'blue';
  ctx.fill();
  ctx.fillStyle = 'black';
...
/*  ctx.font = '40px Arial';
  ...
  ctx.textBaseline = 'middle';
  ctx.textAlign = 'left';
  ctx.fillText(' t',425, 630);  //ctx.fillText(' t',275, 650-2);
  ctx.textAlign = 'center';
  ctx.fillText('x　 t 　　　', 275, 775-2);
  ctx.font = '40px Arial';
  ctx.fillText(' ー   グラフ', 275, 775);
/*  ctx.textBaseline = 'bottom';
  ctx.fillText('位', 50, 420);
  ctx.textBaseline = 'top';
  ctx.fillText('置', 50, 420);*/
  ctx.font = '45px Times New Roman italic';
  ctx.fillText('x', 75, 250);  //ctx.fillText('x', 50, 455);
  ...
  ctx.lineTo(600+30*t,600-(v+acceleration*t*10));
  ctx.strokeStyle = 'red';
  ctx.lineWidth = 5;
  ctx.stroke();
  ctx.strokeStyle = 'black';
  ...
  for(i = 0; i < t; i++){
  ctx.beginPath();
  ctx.arc(600+30*i,600-(v+acceleration*i*10), 5, 0, Math.PI*2, false);
  ctx.fillStyle = 'black';
  ctx.fill();
  }
  ctx.closePath();
  ...
  //v-tグラフにベクトルを描画．矢印の先端
  ctx.beginPath();
  if(0 <= v+acceleration*t*10){
  ctx.moveTo(600+30*t,600-(v+acceleration*t*10)-5);
  ctx.lineTo(600+30*t+15*Math.cos(-Math.PI/3*2),600-(v+acceleration*t*10)-20*Math.sin(-Math.PI/3*2)-5);
  ctx.lineTo(600+30*t+15*Math.cos(-Math.PI/3),600-(v+acceleration*t*10)-20*Math.sin(-Math.PI/3)-5);
}else{
  ctx.moveTo(600+30*t,600-(v+acceleration*t*10)+5);
  ctx.lineTo(600+30*t+15*Math.cos(-Math.PI/3*2),600-(v+acceleration*t*10)+20*Math.sin(-Math.PI/3*2)+5);
  ctx.lineTo(600+30*t+15*Math.cos(-Math.PI/3),600-(v+acceleration*t*10)+20*Math.sin(-Math.PI/3)+5);
  ...
  ctx.textBaseline = 'middle';
  ctx.textAlign = 'left';
  ctx.fillText(' t',925, 630); //ctx.fillText(' t',775, 650-2);
  ctx.textAlign = 'center';
  ctx.fillText('v　 t 　　　', 775, 775-2);
  ctx.font = '40px Arial';
  ctx.fillText(' ー   グラフ', 775, 775);
//  ctx.textBaseline = 'bottom';
//  ctx.fillText('速', 550, 420);
//  ctx.textBaseline = 'top';
//  ctx.fillText('度', 550, 420);
  ctx.font = '45px Times New Roman italic';
  ctx.fillText('v', 575, 250);  //  ctx.fillText('v', 550, 455);
  ctx.font = '45px Times New Roman';
  ctx.textAlign = 'right';
...
//  if(intervalID == 1){
  ...
  //acceleration = document.getElementById('acceleration').value;     //加速度を取得
  //acceleration = 2; //ここでは等速直線運動のみのグラフとする
